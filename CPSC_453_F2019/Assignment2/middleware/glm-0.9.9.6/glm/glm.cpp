...
glmFirstPass(GLMmodel* model, FILE* file)
{
    ...
    group = glmAddGroup(model, const_cast<char *>("default"));
...
void getData(GLMmodel* model, GLuint mode,
    ...
    while (group) {
        for (i = 0; i < group->numtriangles; i++) {
            for (GLint idx = 0; idx < 3; ++idx) {
                ...
                triangle = &T(group->triangles[i]);
                if (mode & GLM_SMOOTH) {
                    GLfloat *temp = &model->normals[3 * triangle->nindices[idx]];
                    normal.x = temp[0];
                    normal.y = temp[1];
                    normal.z = temp[2];
                }
                if (mode & GLM_TEXTURE) {
                    GLfloat *temp = &model->texcoords[2 * triangle->tindices[idx]];
                    uv.x = temp[0];
                    uv.y = temp[1];
                }
                GLfloat *temp = &model->vertices[3 * triangle->vindices[idx]];
                vertex.x = temp[0];
                vertex.y = temp[1];
                vertex.z = temp[2];
